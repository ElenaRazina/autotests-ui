...
class CoursesListPage(BasePage):
    def __init__(self, page: Page):
        ...
        def check_click_edit_course(self, index:int):
            expect(self.course_menu_button.nth(index)).to_be_visible()
            expect(self.course_menu_button.nth(index)).click()
            self.course_edit_menu_item.nth(index).to_be_visible()
            self.course_edit_menu_item.nth(index).click()

        def check_click_delete_course(self, index: int):
            expect(self.course_menu_button.nth(index)).to_be_visible()
            expect(self.course_menu_button.nth(index)).click()
            self.course_delete_menu_item.nth(index).to_be_visible()
